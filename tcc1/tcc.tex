...
% ---------------------------------------------------------------
\begin{resumo}
Este documento apresenta a proposta inicial do Trabalho de Conclusão de Curso (TCC1) desenvolvido no SENAC Santo Amaro. O objetivo deste trabalho é analisar como uma RNN pode auxiliar no planejamento de estoque hopitalares com foco em itens críticos. A justificativa para este estudo baseia-se na importância do planejamento e gestão eficiente de estoques hospitalares, no que refere se a insumos crítico, e como a falta deles podem comprometer o atendimento do paciente. A revisão bibliográfica abrange (RNNs), sensibilidade de modelos de machine learning, previsão de serie temporal e gestão de estoque. Como solução proposta, pretende-se Desenvolver um modelo baseado em RNN para previsão de demanda de itens hospitalares críticos, fazendo uso de dados reais de uma base pública, junto à uma análise de sensibilidade apara avaliar como mudança variáveis e paramêtros afetam o modelo. Este trabalho está organizado em três capítulos que apresentam a introdução ao tema, a revisão bibliográfica que fundamenta a pesquisa, o desenvolvimento com a solução proposta e o cronograma de trabalho, seguidos das referências bibliográficas.

\vspace{\onelineskip}

\noindent
\textbf{Palavras-chave}: redes neurais recorentes (RNNs).
\end{resumo}
...
O gerenciamento de estoques é de grande importância para garantir a qualidade e a eficiência do atendimento ao paciente. Por isso, é essencial ter uma boa disponibilidade de insumos, especialmente os que afetam a vitalidade do paciente. Esses insumos devem ter uma maior priorização no momento do planejamento do estoque.  

Com o aumento da complexidade dos sistemas de saúde e da demanda por insumos hospitalares, o planejamento de estoques se torna um desafio. Por isso, o uso de técnicas de previsão de demanda baseadas em aprendizado de máquina, como RNNs, tem se mostrado promissor no que diz respeito à manipulação de dados temporais.

Tendo isso em vista, este trabalho propõe analisar a aplicação de um RNN na previsão da demanda de itens críticos hospitalares, dando ênfase à análise da sensibilidade do modelo a variações de parâmetros de condições. O objetivo é entender como esse método pode contribuir para aumentar e melhorar a eficiência do planejamento de estoques, reduzindo excesso de insumos e riscos de fatalidade por falta.
% ---------------------------------------------------------------
\section{Contexto}
\label{sec:contexto}
...
Os principais desafios para a gestão de estoque são as variações sazonais, as emergências sanitárias e as mudanças no perfil epidemiológico da população. A falta de material pode afetar diretamente a saúde do paciente, ao passo que o excesso pode causar desperdícios, dado que muitos materiais têm um prazo de validade limitado.
