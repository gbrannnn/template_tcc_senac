...
% Tendo isso em vista, este trabalho propõe analisar a aplicação de uma Rede Neural Recorrente (RNN) na previsão da demanda de itens críticos hospitalares, com destaque para a análise de sensibilidade do modelo a variações nos parâmetros e condições de entrada. A escolha por esse tipo de arquitetura se justifica pela sua capacidade inerente de capturar dependências temporais em séries históricas de consumo, característica essencial em ambientes hospitalares, onde a demanda por insumos pode ser influenciada por sazonalidades, surtos epidemiológicos e variações no perfil de atendimento. O objetivo é compreender como esse método pode contribuir para ampliar e aprimorar a eficiência do planejamento de estoques, reduzindo tanto o excesso de insumos — que implica em custos desnecessários e risco de vencimento — quanto os riscos de desabastecimento, que podem comprometer diretamente a qualidade do atendimento e resultar em desfechos clínicos adversos, incluindo fatalidades evitáveis.
...
Com base no objetivo principal forá definido os objetivos secundários, onde tais descrevem as etpas e ações necessária para realizar o objetivo. 
...
\begin{enumerate}
    ...
    \item Definir e aplicar um protocolo experimental de análise de sensibilidade,como fazer uso de SHAP e LIME, aplicando perturbações controladas aos parâmetros de entrada e hiperparâmetros previamente definidos e registrando, de forma sistemática, o desempenho do modelo sob cada cenário;
